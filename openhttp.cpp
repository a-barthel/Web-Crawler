...
int parseURL( const char * url, char * host, int * port, char * document )
{
	...
	// EDIT : (Andrew Barthel) Added capacity to handle "https://".
	// We can now handle HyperText Transfer Protocol URLs and
	// HyperText Transfer Protocol Secure URLs.
	// TODO: Download OpenSSL - include it && add SSL functionality to
	// prexisting sockets and then https will work.
	const char * httpTag = "http://";
	//const char * httpsTag = "https://";
	int lenHttpTag = strlen(httpTag);
	//int lenHttpsTag = strlen(httpsTag);
	if ( strncmp(url,httpTag, lenHttpTag ) /*&& strncmp(url, httpsTag, lenHttpsTag)*/ ) {
		// Missing"http:" && "https://"
		return 1;
	}
	...
	// Get host part
	char* p;
	//if (strncmp(url, httpTag, lenHttpTag) == 0)
		p = (char *) url + lenHttpTag;
	//if (strncmp(url, httpsTag, lenHttpsTag) == 0)
	//	p = (char *) url + lenHttpsTag;
	char * q = host;
	int i = 0;
	...
	*q = 0;
	...
	// Get port
	// Using HyperText Transfer Protocol - port 80
	/*if (strncmp(url, httpTag, lenHttpTag) == 0)*/ *port = 80;
	// Using HyperText Transfer Protocol Secure - port 433
	//if (strncmp(url, httpsTag, lenHttpsTag) == 0) *port = 433;
